...
func (s *service) Open(env envs.Environment, contact *flows.Contact, topic *flows.Topic, body string, assignee *flows.User, logHTTP flows.HTTPLogCallback) (*flows.Ticket, error) {
	...
	go func() {
		...
		// India: previously SendHistory received the session to compute the
		// "since" cut-off. The upstream Open signature no longer takes a
		// session, so SendHistory falls back to a "one minute ago" cut-off.
		SendHistory(contact.ID(), newFlexChannel, logHTTP, s.restClient, s.redactor)
	}()
...
func SendHistory(contactID flows.ContactID, newFlexChannel *FlexChannel, logHTTP flows.HTTPLogCallback, restClient *Client, redactor stringsx.Redactor) {
